...
LRESULT KMainDlg::OnStart(UINT /*uMsg*/, WPARAM wParam, LPARAM lParam)
{
	...
	m_strTime.Format(BkString::Get(132), _T("0秒"));
	m_strStatus.Format(
		BkString::Get(125), 
		BkString::Get(126), 
		BkString::Get(128), 
...
void KMainDlg::OnTimer(UINT_PTR nIDEvent)
{
	if (nIDEvent == MONITOR_TIMER)
	{
		...
		if (m_dwTime <= 60)
		{
			strTime.Format(_T("%d秒"), m_dwTime);
		}
		else if (m_dwTime < 3600)
		{
			strTime.Format(_T("%d分%d秒"), m_dwTime / 60, m_dwTime % 60);
		}
		else if (m_dwTime < 3600 * 24)
		{
			strTime.Format(_T("%d小时%d分"), m_dwTime / 3600, m_dwTime % 3600 / 60);
		}
